  ...
  factory :dish do
    ...
    before(:create) do |dish, evaluator|
      ...
      unless evaluator.name_1.nil? || Ingredient.find_by(name: evaluator.name_1)
        ...
        create(:dishes_ingredient, dish: dish,
      ...
      unless evaluator.name_2.nil? || Ingredient.find_by(name: evaluator.name_2)
        ts = TinySegmenter.new
        create(:dishes_ingredient, dish: dish,
                                  ingredient: Ingredient.find_or_create_by(name: evaluator.name_2, morphemes: ts.segment(evaluator.name_2).join(',')))
      end

      unless evaluator.name_3.nil? || Ingredient.find_by(name: evaluator.name_3)
        ts = TinySegmenter.new
        create(:dishes_ingredient, dish: dish,
                                  ingredient: Ingredient.find_or_create_by(name: evaluator.name_3, morphemes: ts.segment(evaluator.name_3).join(',')))
      end
