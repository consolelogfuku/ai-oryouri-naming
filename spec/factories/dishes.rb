FactoryBot.define do
  factory :dish do
    ...
    dish_name { nil }
    point { 'point' }
    dish_image { Rack::Test::UploadedFile.new(Rails.root.join('spec/factories/test.png').to_s) }
    state { 0 }
    ...
    before(:create) do |dish, evaluator|
      ...
      unless evaluator.name_1.nil? || Ingredient.find_by(name: evaluator.name_1)
        ...
        create(:dishes_ingredient, dish:,
                                   ingredient: Ingredient.find_or_create_by(name: evaluator.name_1, morphemes: ts.segment(evaluator.name_1).join(',')))
      end

      unless evaluator.name_2.nil? || Ingredient.find_by(name: evaluator.name_2)
        ts = TinySegmenter.new
        create(:dishes_ingredient, dish:,
                                   ingredient: Ingredient.find_or_create_by(name: evaluator.name_2, morphemes: ts.segment(evaluator.name_1).join(',')))
      end

      unless evaluator.name_3.nil? || Ingredient.find_by(name: evaluator.name_3)
        ts = TinySegmenter.new
        create(:dishes_ingredient, dish:,
                                   ingredient: Ingredient.find_or_create_by(name: evaluator.name_3, morphemes: ts.segment(evaluator.name_1).join(',')))
      end
      ...
      cooking_methods.each do |cooking_method|
        create(:dishes_cooking_method, dish:, cooking_method:) # 関連も一緒に保存される
      end
    end
  end
end
